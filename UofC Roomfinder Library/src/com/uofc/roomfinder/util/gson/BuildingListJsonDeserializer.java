...
public class BuildingListJsonDeserializer implements JsonDeserializer<List<Building>> {
	...
	public List<Building> deserialize(JsonElement json, Type arg1, JsonDeserializationContext arg2) throws JsonParseException {
		...
		for (int i = 0; i < featureArray.size(); i++) {
			...
			if (attributes.get(BUILDING_NAME_COL) != null)
				newBuilding.setName_2(attributes.get(BUILDING_NAME_COL).getAsString());

			if (!attributes.get(BUILDING_ADDRESS_COL).isJsonNull())
				newBuilding.setAddress(attributes.get(BUILDING_ADDRESS_COL).getAsString());

			if (!attributes.get(BUILDING_USE_COL).isJsonNull())
				newBuilding.setUse(attributes.get(BUILDING_USE_COL).getAsString());

			if (!attributes.get(BUILDING_ID_COL).isJsonNull())
				newBuilding.setAbbreviation(attributes.get(BUILDING_ID_COL).getAsString());
			...
			lat = geometryElements.get(0).getAsJsonArray().get(0).getAsDouble();
			lng = geometryElements.get(0).getAsJsonArray().get(1).getAsDouble();
